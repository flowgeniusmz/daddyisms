...
### Variables - CSV path (will be created if missing)
csv_path = "journal.csv"

# Functions
def get_result_emoji_color(entry_price, exit_price):
    ...
    if type == "w": 
        ...
        emoji = "⚪"
        color = 0xcccccc
    return result, emoji, color, percent_change

def send_alerts(alert, entry_price):
    urls = alert_urls
    payload = {"content": f"@everyone {alert} | Entry Price: ${entry_price:.2f}"}
    for url in urls:
...
### Display 1 - Send Alert
st.subheader("1. Send Trade Alert")
alert = st.text_input(label="Alert")
entry_price = st.number_input(label="Entry Price", min_value=0, format="%.2f")
btn_alert = st.button(label="Send Alert")
if btn_alert:
    send_alerts(alert=alert, entry_price=entry_price)
    st.divider()

    st.subheader("2. Complete Journal Entry")
    exit_price = st.number_input(label="Exit Price",min_value=0, format="%.2f" )
    percent_change = None
    result = None
    emoji = ""
    color = 0
